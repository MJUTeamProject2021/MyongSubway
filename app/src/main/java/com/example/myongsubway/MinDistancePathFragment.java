...
public class MinDistancePathFragment extends Fragment {
    ArrayList<Integer> minDistancePath;                 // 최소시간을 기준으로 탐색한 경로
    ...
    public MinDistancePathFragment(ArrayList<Integer> path, ArrayList<Integer> _lines, ArrayList<Integer> _costs, CustomAppGraph _graph, ArrayList<Integer> _btnBackgrounds, ArrayList<Integer> _lineColors) {
    ...
    public void onResume() {
        ((ShortestPathActivity) getActivity()).setPageType(CustomAppGraph.SearchType.MIN_DISTANCE);
        super.onResume();
    }
    ...
    private void init(View v) {
        ...
        // 출발역, 도착역의 Vertex 객체
        CustomAppGraph.Vertex departure = graph.getVertices().get(minDistancePath.get(0));
        CustomAppGraph.Vertex arrival = graph.getVertices().get(minDistancePath.get(minDistancePath.size() - 1));
            ...
            public void onClick(View v) {
                switch (v.getId()) {
                    case R.id.departureButton:
                        ((ShortestPathActivity) getActivity()).generateStationInformationFragment(graph.getVertices().get(minDistancePath.get(0)));
                        break;

                    case R.id.arrivalButton:
                        ((ShortestPathActivity) getActivity()).generateStationInformationFragment(graph.getVertices().get(minDistancePath.get(minDistancePath.size() - 1)));
                        break;
